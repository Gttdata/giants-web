...
export class UnitReportComponent implements OnInit {
  columns: string[][] = [
    ...
    ["GROUP_COUNT", "NO of Group "], ["MEMBER_COUNT", "No. Of Member"], ["MEETING_COUNT", "No.Of Meeting"],
  ...
  formTitle: string = "Unit Level Report";
  ...
  SelectedYear: any
  tagValue: string[] = ["Select All", "Federation Name", "Name Of Unit", "NO of Group", "No. Of Member", "No.Of Meeting", "Meeting Planned", "Meeting Conducted", "Avg. Attendence", "Meeting Time", "Avg. Meeting/day", "Avg. Meetings/Group", "No. Of Post", "Total Likes", "Total Comments", "Avg. Post/Member", "Avg. Like/Member", "Avg. Comment/Member", "No. Of Shares", "No. Of Event", "Total event Planned", "Event Conducted", "Top Types", "No. Of Circular", "Total Circular Created", "Published / Signed", "Total View's", "Avg. View/Circular", "No. Of Project", "Completed Project's", "Ongoing Project", "Total Fund Utilisation", "Avg. Fund/Project", "Avg. Projects/group"]
  ...
  search(reset: boolean = false, exportToExcel: boolean = false, exportToPDF: boolean = false) {
    ...
    if (exportToExcel) {
      ...
      this.api.getUnitReport(0, 0, this.sortKey, this.sortValue, "", this.SelectedYear, filters, this.federationId, this.UnitId, this.GroupId).subscribe(data => {
        if (data['code'] == 200) {
      ...
      this.api.getUnitReport(this.pageIndex, this.pageSize, this.sortKey, this.sortValue, "", this.SelectedYear, filters, this.federationId, this.UnitId, this.GroupId).subscribe(data => {
        if (data['code'] == 200) {
  ...
  convertInExcel() {
    ...
    for (var i = 0; i < this.dataListForExport.length; i++) {
      ...
      if (i == this.dataListForExport.length - 1) {
        this._exportService.exportExcel(arry1, 'Unit Wise Count ' + this.datePipe.transform(new Date(), 'dd-MMM-yy, hh mm ss a'));
      }
    }
  }
  getFederation() {
    ...
    this.api.getUnitReport(this.pageIndex, this.pageSize, this.sortKey, this.sortValue, "", this.SelectedYear, "", this.federationId, this.UnitId, this.GroupId).subscribe(data => {
      if ((data['code'] == 200)) {
        this.federations = data['data'];
        this.totalRecords = data['count'];
        this.loadingRecords = false;
  ...
  getUnitReport() {
    ...
    this.api.getUnitReport(this.pageIndex, this.pageSize, "FEDERATION_NAME", "asc", "", this.SelectedYear, this.all_filter, this.federationId, this.UnitId, this.GroupId).subscribe(data => {
      if ((data['code'] == 200)) {
        this.federations = data['data'];
        this.totalRecords = data['count'];
        this.loadingRecords = false;
  ...
  goToSchedule(reset: boolean = false, loadMore: boolean = false): void {
    this.ScheduleTitle = "Unit Report";
