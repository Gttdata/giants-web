...
@Component({
  selector: 'app-unit-wise-monthly-report-details',
  templateUrl: './unit-wise-monthly-report-details.component.html',
  styleUrls: ['./unit-wise-monthly-report-details.component.css']
})

export class UnitWiseMonthlyReportDetailsComponent implements OnInit {
  formTitle: string = "Unit Wise Monthly Reporting Submission Count";
  dataList: any[] = [];
  ...
  sortKey: string = "UNIT_NAME";
  searchText: string = "";
  isFilterApplied: any = "default";
  filterClass: string = 'filter-invisible';
  columns: string[][] = [["UNIT_NAME", "Unit Name"]];
  monthColumns: any[] = [];
  ...
  search(reset: boolean = false): void {
    ...
    this.api.getUnitWiseMonthlyReportDetails(this.pageIndex, this.pageSize, this.sortKey, sort, likeQuery + unitNameFilter + federationFilter, new Date().getFullYear()).subscribe(data => {
      if (data['code'] == 200) {
