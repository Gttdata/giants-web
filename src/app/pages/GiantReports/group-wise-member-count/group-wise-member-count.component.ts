...
@Component({
  selector: 'app-group-wise-member-count',
  templateUrl: './group-wise-member-count.component.html',
  styleUrls: ['./group-wise-member-count.component.css']
})

export class GroupWiseMemberCountComponent implements OnInit {
  passwordVisible: boolean = false;
  formTitle: string = "Group Wise Member Count";
  pageIndex: number = 1;
  pageSize: number = 10;
  ...
  columns1: string[][] = [["FEDERATION_NAME", "Federation"], ["UNIT_NAME", "Unit"], ["GROUP_NAME", "Group"]];
  ...
  search(reset: boolean = false, exportToExcel: boolean = false, exportToPDF: boolean = false): void {
    ...
    if (exportToExcel) {
      ...
      this.api.getGroupWiseMemberCount(0, 0, this.sortKey, sort, likeQuery + federationFilter + unitFilter + groupFilter + federationFilterStr + unitFilterStr + groupFilterStr + " AND ACTIVE_STATUS='A'").subscribe(data => {
        if (data['code'] == 200) {
      ...
      this.api.getGroupWiseMemberCount(this.pageIndex, this.pageSize, this.sortKey, sort, likeQuery + federationFilter + unitFilter + groupFilter + federationFilterStr + unitFilterStr + groupFilterStr + " AND ACTIVE_STATUS='A'").subscribe(data => {
        if (data['code'] == 200) {
  ...
  getGenderFullForm(gender): string {
    if (gender == "M")
      return "Male";
  ...
  getMaritalStatusFullForm(gender): string {
    if (gender == "S")
      return "Single";
  ...
  getActiveStatus(status: any): string {
    if (status == "P")
      return "Pending";
  ...
  applyFilter(): void {
    if ((this.FEDERATION_ID.length > 0) || (this.UNIT_ID.length > 0) || (this.GROUP_ID.length > 0))
  ...
  convertInExcel(): void {
    ...
    for (var i = 0; i < this.dataListForExport.length; i++) {
      ...
      if (i == this.dataListForExport.length - 1) {
        this._exportService.exportExcel(arry1, 'Group Wise Member Count ' + this.datePipe.transform(new Date(), 'dd-MMM-yy, hh mm ss a'));
      }
    }
  }
}
